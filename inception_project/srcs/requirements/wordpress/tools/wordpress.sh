...
if [ ! -f "/allready.txt" ]; then
    ...
    touch /allready.txt
    mkdir /var/www/html/wordpress -p
    ...
    if [ ! -f "/var/www/html/wordpress/wp-config.php" ]; then	
        ...
        wp core install --url="$ip" --title="Arctic monkeys" --admin_user=$ADMIN_USER --admin_password=$ADMIN_PASSWORD --admin_email=$ADMIN_EMAIL --allow-root
